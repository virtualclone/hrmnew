...
<template>
  <div class="orangehrm-background-container">
    <div class="orangehrm-card-container">
      ...
      <oxd-form :loading="isLoading" @submit-valid="onSave">
        <oxd-grid :cols="2" class="orangehrm-full-width-grid">
          <oxd-grid-item>
            <oxd-input-field v-model="project.name" :label="$t('general.name')" :rules="rules.name" required />
          </oxd-grid-item>
          <oxd-grid-item>
            <customer-autocomplete :key="project.customer" v-model="project.customer" :rules="rules.customer"
              :label="$t('time.customer_name')" required />
            <oxd-button icon-name="plus" display-type="text" class="orangehrm-input-field-bottom-space"
              :label="$t('time.add_customer')" @click="onClickAddCustomer" />
          </oxd-grid-item>
        </oxd-grid>
        <oxd-form-row>
          <oxd-grid :cols="2" class="orangehrm-full-width-grid">
            <oxd-grid-item>
              <oxd-input-field v-model="project.description" type="textarea" :label="$t('general.description')"
                :rules="rules.description" :placeholder="$t('general.type_description_here')" />
            </oxd-grid-item>
            <oxd-grid-item>
              <project-admin-autocomplete v-for="(projectAdmin, index) in projectAdmins" :key="index"
                v-model="projectAdmin.value" :show-delete="index > 0" :rules="rules.projectAdmin"
                include-employees="onlyCurrent" @remove="onRemoveAdmin(index)" />
              <oxd-button v-if="projectAdmins.length < 5" icon-name="plus" display-type="text"
                :label="$t('general.add_another')" @click="onAddAnother" />
            </oxd-grid-item>

            <oxd-grid-item>
              <oxd-input-field v-model="project.forall" type="checkbox" value="true" :label="$t('For All')"
    ...
    <add-customer-modal v-if="showCustomerModal" @close="onCustomerModalClose"></add-customer-modal>
  </div>
</template>
