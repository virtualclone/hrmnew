  ...
  >
      ...
      <oxd-form :loading="isLoading" @submit-valid="onSave">
        <oxd-form-row>
          <oxd-grid :cols="3" class="orangehrm-full-width-grid">
            ...
            <oxd-grid-item>
              <workweek-dropdown v-model="job.workweekId"/>
            </oxd-grid-item>
            <oxd-grid-item>
              <oxd-input-field
                v-model="job.noticeperiod"
                type="input"
    ...
    <oxd-form :loading="isLoading" @submit-valid="onSubmit">
      <oxd-form-row>
        ...
        <oxd-grid-item>
          <date-input
                v-model="job.lastreviewdate"
                :label="$t('Last Increment')"
                :rules="rules.lastreviewdate"
                :disabled="true"
              />
        </oxd-grid-item>
        <oxd-grid-item>
          <date-input
                v-model="job.nextreviewdate"
                :label="$t('Next Increment')"
                :disabled="!hasUpdatePermissions"
              />
        </oxd-grid-item>
      </oxd-grid>
...
export default {
  ...
  methods: {
    ...
    updateJobModel(response) {
      ...
      this.job.workshiftId = this.workshifts.find(
        (item) => item.id === data.workshift?.id,
      );
