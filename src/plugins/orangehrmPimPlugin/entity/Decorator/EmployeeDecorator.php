...
class EmployeeDecorator
{
    ...
    {
        return $this->getEmployee()->getGender();
    }
...
//Added new for workshift
        /**
     * @return WorkShift|null
     */
    public function getWorkshift(): ?WorkShift
    {
        $workshifts = $this->getEmployee()->getWorkshifts();
        if (empty($workshifts) || !isset($workshifts[0])) {
            return null;
        }
        return $workshifts[0];
    }
     ...
     * @param int|null $id
     */
    public function setWorkshiftById(?int $id): void
    {
        $workshift = $this->getWorkshift();
        $workshiftId = $workshift instanceof WorkShift ? $workshift->getId() : null;

        if (is_null($id)) {
            // Remove workshift
            $this->getEmployee()->setWorkshifts([]);
        } elseif ($workshiftId !== $id) {
            // Changed workshift
            $this->getEmployee()->setWorkshifts([]);

            $workshift = $this->getReference(WorkShift::class, $id);
            if ($workshift) {
                $this->getEmployee()->setWorkshifts([$workshift]);
            }
        } // else not changed workshift
    }
