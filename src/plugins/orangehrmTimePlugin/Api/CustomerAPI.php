...
class CustomerAPI extends Endpoint implements CrudEndpoint
{
    ...
    public const PARAMETER_EMAILTO = 'emailto';
    public const PARAMETER_MHRSLIMIT = 'mhrslimit';
    public const PARAMETER_ACTIVE = 'active';  

    public const FILTER_NAME = 'name';
    public const FILTER_INCLUDE_STATUS = 'status';
    ...
    public function getAll(): EndpointResult
    {
        ...
        $customerSearchParamHolder->setIncludeStatus(
            $this->getRequestParams()->getStringOrNull(
                RequestParams::PARAM_TYPE_QUERY,
                self::FILTER_INCLUDE_STATUS
            )

        );
        $customerSearchParamHolder->setCustomerId(
    ...
    public function getValidationRuleForGetAll(): ParamRuleCollection
    {
        return new ParamRuleCollection(
            ...
            new ParamRule(self::FILTER_INCLUDE_STATUS),
    ...
    public function update(): EndpointResult
    {
        ...
        $customer = $this->getCustomerService()->getCustomer($customerId);
        $customerActive = $this->getRequestParams()->getInt(RequestParams::PARAM_TYPE_BODY, self::PARAMETER_ACTIVE);
        if($customerActive== false)
        {
            $hasTimesheetItems = $this->getCustomerService()->getCustomerDao()->hasCustomerGotTimesheetItemsStatus($customerId);
            if ($hasTimesheetItems) {
                throw new BadRequestException('Not Allowed to Deactivate Customer Which Have Time Logged Against Them');
            }
            $this->getCustomerService()->getCustomerDao()->updateProjectRelatedtoCustomer($customerId);
        }
        $this->throwRecordNotFoundExceptionIfNotExist($customer, Customer::class);
        $this->setParamsToCustomer($customer);
        $this->getCustomerService()
    ...
    protected function getCommonBodyValidationRules(): array
    {
        return [
            ...
            $this->getValidationDecorator()->notRequiredParamRule(
                new ParamRule(
                    self::PARAMETER_EMAILTO,
                ),
                true  
            ),
            $this->getValidationDecorator()->requiredParamRule(
                new ParamRule(
                    self::PARAMETER_MHRSLIMIT,
                ),
                true
            ),
            $this->getValidationDecorator()->requiredParamRule(
    ...
    private function setParamsToCustomer(Customer $customer): void
    {
        ...
        $customerEmail = $this->getRequestParams()->getString(
            RequestParams::PARAM_TYPE_BODY,
            self::PARAMETER_EMAILTO
        );
        $customerHrsLimit = $this->getRequestParams()->getIntOrNull(RequestParams::PARAM_TYPE_BODY, self::PARAMETER_MHRSLIMIT);
        $customerActive = $this->getRequestParams()->getInt(RequestParams::PARAM_TYPE_BODY, self::PARAMETER_ACTIVE);
        $customer->setClientEmail($customerEmail);
        $customer->setCustduration($customerHrsLimit);
        $customer->setActivestatus($customerActive);
