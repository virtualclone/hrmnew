...
class ProjectDao extends BaseDao
{
    ...
    protected function getProjectsPaginator(ProjectSearchFilterParams $projectSearchFilterParamHolder): Paginator
    {
        ...
        if (!is_null($projectSearchFilterParamHolder->getUserNumber())) {
        $qb->leftJoin('project.projectUsers', 'projectUser');
        }
       // $qb->leftJoin('project.projectuser', 'projectUser');
        ...
        if (!is_null($projectSearchFilterParamHolder->getUserNumber())) {
            $qb->andWhere('projectUser.empNumber = :userNumber')
                ->setParameter('userNumber', $projectSearchFilterParamHolder->getUserNumber());
        }
        ...
        if (
            is_null($projectSearchFilterParamHolder->getIncludeStatus()) ||
            $projectSearchFilterParamHolder->getIncludeStatus() ===
            ProjectSearchFilterParams::INCLUDE_CUSTOMER_ONLY_ACTIVE
        ) {
            $qb->andWhere('project.activestatus = :activestatus');
            $qb->setParameter('activestatus', 1);
        } elseif (
            $projectSearchFilterParamHolder->getIncludeStatus() ===
            ProjectSearchFilterParams::INCLUDE_CUSTOMER_ONLY_INACTIVE
        ) {
            $qb->andWhere('project.activestatus = :activestatus');
            $qb->setParameter('activestatus', 0);
        }    
     ...
     * @return array
     */
    public function getEmpByProjectId($projectId): array
    {
        $qb = $this->createQueryBuilder(ProjectAdmin::class, 'pa');
        $qb->andWhere('pa.projectId = :projectId');
        $qb->setParameter('projectId', $projectId);
        $result = $qb->getQuery()->getArrayResult();
        $finalResult = [];
        foreach ($result as $item) {
            $empNumber = $item['emp_number'];
            $qb = $this->createQueryBuilder(Employee::class, 'emp');
            $qb->andWhere('emp.empNumber = :empNumber');
        ...
        // print_r($finalResult);
        return $finalResult; //array_column($finalResult, 'id');
    }
}
