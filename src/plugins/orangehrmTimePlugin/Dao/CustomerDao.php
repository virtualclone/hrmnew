...
class CustomerDao extends BaseDao
{
    ...
    private function getSearchCustomerPaginator(CustomerSearchFilterParams $customerSearchFilterParams): Paginator
    {
        ...
        if (!is_null($customerSearchFilterParams->getCustomerId())) {
            $q->andWhere($q->expr()->in('customer.id', ':customerIds'))
                ->setParameter('customerIds', $customerSearchFilterParams->getCustomerId());
        }
        if (is_null($customerSearchFilterParams->getIncludeStatus()) ||
            $customerSearchFilterParams->getIncludeStatus() ===
            CustomerSearchFilterParams::INCLUDE_CUSTOMER_ONLY_ACTIVE
        ) {
            $q->andWhere('customer.activestatus = :activestatus');
            $q->setParameter('activestatus', 1);
        } elseif (
            $customerSearchFilterParams->getIncludeStatus() ===
            CustomerSearchFilterParams::INCLUDE_CUSTOMER_ONLY_INACTIVE
        ) {
            $q->andWhere('customer.activestatus = :activestatus');
            $q->setParameter('activestatus', 0);
        }    
     ...
     * @return bool
     */
     public function hasCustomerGotTimesheetItemsStatus(int $customerId): bool
     { 
         $state='APPROVED'; 
         $q = $this->createQueryBuilder(TimesheetItem::class, 'timesheetItem'); 
         $q->leftJoin('timesheetItem.project', 'project'); 
     ...
     public function updateProjectRelatedtoCustomer(int $customerId): int
    {
        $q = $this->createQueryBuilder(Project::class, 'project');
        $q->update()
            ->set('project.activestatus', ':activestatus')
            ->setParameter('activestatus', false)
            ->where($q->expr()->in('project.customer', ':customerId'))
